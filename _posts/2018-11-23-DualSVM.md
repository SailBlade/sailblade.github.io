...
##  背景   
对偶SVM分类相比线性SVM分类难度有所提升，实践的时候对于二次规划入参的分解还存在困难，另外对于Z=phi(x)也理解不够，不能解决分类问题。    


##  Dual SVM 分类范例   
#### 问题描述：   
利用linear SVM 对已知的线性数据进行分类 `Y = sign(w1*X1*X1 + w2*X2*X2 + b)`。待训练数据格式如下：    
...
for loop in range(Y.shape[0]):
	Y[loop] = self.__sign__(W1 * X1[loop]*X1[loop] + W2 * X2[loop] * X2[loop] + b)
```   


可视化如下：   
![](https://portal.qiniu.com/bucket/sailbladespace/resource)    
